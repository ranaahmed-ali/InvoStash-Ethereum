...
contract StaticStakingContract is Governable {
    ...
    uint256[4] public durations = [15 seconds,30 seconds,45 seconds,60 seconds]; // allowed durations


    //////// Change rates with 2 , according to your needs /////////
    // Just give percentage E.g : 2 => 2% || 45 => 45 %         <----------------------- KEY POINT
    uint256[4] public rates = [4, 6, 8, 9]; // rates that correspond with the allowed durations
    ...
    // type 0 is reserved for stakes done by the user
        ...
        returns (uint256 total)
    {
        for (uint256 i = 0; i < stakes.length; i++) {
            Stake storage stake1 = stakes[i];
            if (!stake1.paid) {
                total = total.add(stake1.amount.mul(stake1.rate.mul(1e2)).div(10000));
            }
        }
    }

    function _totalExpected(Stake storage _stake2)
        internal
        view
        returns (uint256)
    {
     ...
     * @param rate rates[0 => 4%] of reward for this stake in 1e18, uint240 =
        ...
        returns (uint256 total)
    {
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            Stake storage stake3 = stakes[i];
            if (stake3.paid) {
                continue;
            } else if (stake3.end < block.timestamp) {
                total = total.add(_totalExpected(stake3));
            } else {
                //calcualte the precentage accrued in term of rewards
                total = total.add(
                    stake3.amount.add(
                        stake3.amount.mulTruncate(stake3.rate).mulTruncate(
                            stake3
                                .duration
                                .sub(stake3.end.sub(block.timestamp))
                                .divPrecisely(stake3.duration)
                        )
                    )
                );
            }
    ...
    function Unstake(uint256 _stakeId) external requireLiquidity {
        ...
        Stake storage exitStake = stakes[_stakeId - 1];

        require(exitStake.paid == false, "already unstaked");
        require(exitStake.end < block.timestamp, "You cannot unstake before time");
        //might not be ended
        //we are paying out the stake
        exitStake.paid = true;
        totalWithdraw = totalWithdraw.add(_totalExpected(exitStake));
